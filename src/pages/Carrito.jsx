  ...
  return (
    <>
      ...
      {carrito.length === 0 ? (
        ...
        carrito.map((item) => <CarritoCard producto={item} />)
      )}
      <div className="botones-abajo">
        <button onClick={handleBorrarCarrito} className="boton-secundario" disabled={carrito.length == 0}>
          Borrar carrito
        </button>
        <button onClick={handleComprar} className="boton-primario" disabled={carrito.length == 0}>
          Finalizar compra
        </button>
      </div>
    </>
